...
describe('GET /hotels/all', () => {
  it('should respond with status 401 if no token is given', async () => {
    const response = await server.get('/hotels/all');
  ...
  it('should respond with status 401 if given token is not valid', async () => {
    ...
    const response = await server.get('/hotels/all').set('Authorization', `Bearer ${token}`);
  ...
  describe('when token is valid', () => {
    ...
    it('should respond with status 200 and hotels with rooms', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
      ...
      const createdRoom = await createRoomWithHotelId(createdHotel.id);
      const createdRoom2 = await createRoomWithHotelId(createdHotel2.id);

      const createdBooking = await createBooking({ userId: user.id, roomId: createdRoom.id });
      ...
      expect(response.body).toEqual([
        ...
        {
          id: createdHotel2.id,
          name: createdHotel2.name,
          image: createdHotel2.image,
          createdAt: createdHotel2.createdAt.toISOString(),
          updatedAt: createdHotel2.updatedAt.toISOString(),
          Rooms: [
            {
              id: createdRoom2.id,
              name: createdRoom2.name,
              capacity: createdRoom2.capacity,
              hotelId: createdHotel2.id,
